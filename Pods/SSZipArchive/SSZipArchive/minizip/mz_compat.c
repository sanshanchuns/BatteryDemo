...
zipFile mz_zipOpen(const char *path, int append)
{
    zlib_filefunc64_def pzlib = mz_stream_os_get_interface();
    return mz_zipOpen2(path, append, NULL, &pzlib);
}

zipFile mz_zipOpen64(const void *path, int append)
{
    zlib_filefunc64_def pzlib = mz_stream_os_get_interface();
    return mz_zipOpen2(path, append, NULL, &pzlib);
}

zipFile mz_zipOpen2(const char *path, int append, const char **globalcomment,
    zlib_filefunc_def *pzlib_filefunc_def)
{
    return mz_zipOpen2_64(path, append, globalcomment, pzlib_filefunc_def);
}

zipFile mz_zipOpen2_64(const void *path, int append, const char **globalcomment,
    zlib_filefunc64_def *pzlib_filefunc_def)
{
    ...
    zip = mz_zipOpen_MZ(stream, append, globalcomment);
...
zipFile mz_zipOpen_MZ(void *stream, int append, const char **globalcomment)
{
...
int mz_zipOpenNewFileInZip5(zipFile file, const char *filename, const zip_fileinfo *zipfi,
...
int mz_zipWriteInFileInZip(zipFile file, const void *buf, uint32_t len)
{
    mz_compat *compat = (mz_compat *)file;
    int32_t written = 0;
...
int mz_zipCloseFileInZipRaw(zipFile file, uint32_t uncompressed_size, uint32_t crc32)
{
    return mz_zipCloseFileInZipRaw64(file, uncompressed_size, crc32);
}

int mz_zipCloseFileInZipRaw64(zipFile file, int64_t uncompressed_size, uint32_t crc32)
{
    mz_compat *compat = (mz_compat *)file;
    if (compat == NULL)
        return ZIP_PARAMERROR;
...
int mz_zipCloseFileInZip(zipFile file)
{
    return mz_zipCloseFileInZip64(file);
}

int mz_zipCloseFileInZip64(zipFile file)
{
    mz_compat *compat = (mz_compat *)file;
    if (compat == NULL)
        return ZIP_PARAMERROR;
...
int mz_zipClose(zipFile file, const char *global_comment)
{
    return mz_zipClose_64(file, global_comment);
}

int mz_zipClose_64(zipFile file, const char *global_comment)
{
    return mz_zipClose2_64(file, global_comment, MZ_VERSION_MADEBY);
}

int mz_zipClose2_64(zipFile file, const char *global_comment, uint16_t version_madeby)
{
    mz_compat *compat = (mz_compat *)file;
    int32_t err = MZ_OK;

    if (compat->handle != NULL)
        err = mz_zipClose2_MZ(file, global_comment, version_madeby);
...
int mz_zipClose_MZ(zipFile file, const char *global_comment)
{
    return mz_zipClose2_MZ(file, global_comment, MZ_VERSION_MADEBY);
}

/* Only closes the zip handle, does not close the stream */
int mz_zipClose2_MZ(zipFile file, const char *global_comment, uint16_t version_madeby)
{
    mz_compat *compat = (mz_compat *)file;
    int32_t err = MZ_OK;
