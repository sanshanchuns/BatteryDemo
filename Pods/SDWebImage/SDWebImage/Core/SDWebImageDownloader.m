...
- (nullable NSOperation<SDWebImageDownloaderOperation> *)createDownloaderOperationWithUrl:(nonnull NSURL *)url
    ...
    NSNumber *timeoutNumber = context[SDWebImageContextLoaderTimeoutInterval];
    if (timeoutNumber && [timeoutNumber isKindOfClass:[NSNumber class]]) {
        timeoutInterval = timeoutNumber.doubleValue;
    } else {
        timeoutInterval = self.config.downloadTimeout;
    }
...
- (id<SDWebImageOperation>)requestImageWithURL:(NSURL *)url options:(SDWebImageOptions)options context:(SDWebImageContext *)context progress:(SDImageLoaderProgressBlock)progressBlock completed:(SDImageLoaderCompletedBlock)completedBlock {
    ...
    if (options & SDWebImageReturnOnlyImageData) downloaderOptions |= SDWebImageDownloaderReturnOnlyImageData;
    if (options & SDWebImageImmediateCallCompletionBlock) downloaderOptions |= SDWebImageDownloaderImmediateCallCompletionBlock;
