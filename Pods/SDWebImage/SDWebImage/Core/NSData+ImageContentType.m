...
#define kSDUTTypeKPG ((__bridge CFStringRef)@"kuaishou.kpg")
    ...
    switch (c) {
        ...
        case 0x52: {
            if (data.length >= 12) {
                ...
                } else if([testString hasPrefix:@"RIFF"] && [testString hasSuffix:@"KPGB"]){
                    return SDImageFormatKPG;
                }
            }
            break;
        }
...
+ (SDImageFormat)sd_imageFormatFromUTType:(CFStringRef)uttype {
    ...
    } else if (CFStringCompare(uttype, kSDUTTypeKPG, 0) == kCFCompareEqualTo) {
        imageFormat = SDImageFormatKPG;
