...
static int32_t kSDShouldDecodeImage = 1;
static int32_t kSDCopyAttributeRunCount = 0;
...
+ (void)applicationWillTerminate:(NSNotification *)note {
    OSAtomicCompareAndSwap32Barrier(1, 0, &kSDShouldDecodeImage);
    
    // FYI: wait until all CGImageSourceCopyProperties return
    while (!OSAtomicCompareAndSwap32Barrier(0, INT32_MIN, &kSDCopyAttributeRunCount)) {
...
+ (NSTimeInterval)frameDurationAtIndex:(NSUInteger)index source:(CGImageSourceRef)source {
    ...
    OSAtomicIncrement32Barrier(&kSDCopyAttributeRunCount);
    if (kSDCopyAttributeRunCount <= 0) {
        return frameDuration;
    }
    CFDictionaryRef cfFrameProperties = CGImageSourceCopyPropertiesAtIndex(source, index, (__bridge CFDictionaryRef)options);
    OSAtomicDecrement32Barrier(&kSDCopyAttributeRunCount);
...
+ (UIImage *)createFrameAtIndex:(NSUInteger)index source:(CGImageSourceRef)source scale:(CGFloat)scale preserveAspectRatio:(BOOL)preserveAspectRatio thumbnailSize:(CGSize)thumbnailSize forceDecode:(BOOL)forceDecode options:(NSDictionary *)options {
    ...
    CGImageRef imageRef;
    BOOL createFullImage = thumbnailSize.width == 0 || thumbnailSize.height == 0 || pixelWidth == 0 || pixelHeight == 0 ||
    ...
    if (createFullImage) {
        ...
        if (preserveAspectRatio) {
            CGFloat pixelRatio = (CGFloat)pixelWidth / pixelHeight;
            CGFloat thumbnailRatio = thumbnailSize.width / thumbnailSize.height;
            if (fabs(pixelRatio - thumbnailRatio) < FLT_EPSILON) {
                maxPixelSize = MAX(thumbnailSize.width, thumbnailSize.height);
            } else if (pixelRatio > thumbnailRatio) {
                maxPixelSize = MAX(thumbnailSize.width, ceil(thumbnailSize.width / pixelRatio));
            } else {
                maxPixelSize = MAX(ceil(thumbnailSize.height * pixelRatio), thumbnailSize.height);
            }
        } else {
            maxPixelSize = MAX(thumbnailSize.width, thumbnailSize.height);
        }
        ...
        NSLog(@"SDImageIOAnimatedCoder thumbnailSize.width:%ld thumbnailSize.height:%ld pixelWidth:%ld pixelHeight:%ld", (long)thumbnailSize.width, (long)thumbnailSize.height, pixelWidth, pixelHeight);
...
- (UIImage *)decodedImageWithData:(NSData *)data options:(nullable SDImageCoderOptions *)options {
    ...
    animatedImage.sd_imageDecoder = NSStringFromClass(self.class);
    CFRelease(source);
