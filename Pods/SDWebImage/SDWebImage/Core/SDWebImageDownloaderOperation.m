...
didReceiveResponse:(NSURLResponse *)response
 completionHandler:(void (^)(NSURLSessionResponseDisposition disposition))completionHandler {
    ...
    if (SDWebImageDownloaderConfig.defaultDownloaderConfig.imageOpenLargeFileDetect) {
        if (self.expectedSize >= SDWebImageDownloaderConfig.defaultDownloaderConfig.expectedSizeThreshold) {
            NSAssert(NO, @"ExpectedSize异常，可能引发OOM，需要业务方确认是否为正常的图片URL请求");
            valid = NO;
            self.responseError = [NSError errorWithDomain:SDWebImageErrorDomain code:SDWebImageErrorBadImageData userInfo:@{NSLocalizedDescriptionKey : @"ExpectedSize is larger than 1G, more likely to cause OOM"}];
        }
    }
...
- (void)URLSession:(NSURLSession *)session dataTask:(NSURLSessionDataTask *)dataTask didReceiveData:(NSData *)data {
    NSLog(@"SDWebImageDownloader downloaded image with url : %@", self.request.URL);
    if (!self.imageData) {
        ...
        if ([self callbacksForKey:kCompletedCallbackKey].count > 0) {
            ...
            if (imageData) {
                ...
                if (self.options & SDWebImageDownloaderIgnoreCachedResponse && [self.cachedData isEqualToData:imageData]) {
                    ...
                    [self done];
                } else if (self.options & SDWebImageDownloaderReturnOnlyImageData) {
                    ...
                    [self.coderQueue addOperationWithBlock:^{
                        ...
                        if (imageSize.width == 0 || imageSize.height == 0) {
                            ...
                            NSError *decodeError = [NSError errorWithDomain:SDWebImageErrorDomain
                                                                 code:SDWebImageErrorBadImageData
                                                             userInfo:@{NSLocalizedDescriptionKey : description,
                                                                        SDWebImageErrorImageFormatKey : @([NSData sd_imageFormatForImageData:imageData])}];
                            [self callCompletionBlocksWithError:decodeError];
                        } else {
