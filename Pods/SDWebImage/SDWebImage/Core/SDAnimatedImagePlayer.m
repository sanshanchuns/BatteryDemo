...
@property (nonatomic, assign) BOOL loopEnding;
...
- (void)setupCurrentFrame {
    ...
    if (!self.currentFrame && [self.animatedProvider isKindOfClass:[UIImage class]]) {
        ...
        if (self.playbackMode == SDAnimatedImagePlaybackModeReverse ||
                   self.playbackMode == SDAnimatedImagePlaybackModeReversedBounce) {
            image = [self.animatedProvider animatedImageFrameAtIndex:self.currentFrameIndex];
        } else {
            image = (UIImage *)self.animatedProvider;
        }
...
- (void)displayDidRefresh:(SDDisplayLink *)displayLink {
    ...
    if (!self.bufferMiss) {
        ...
        switch (self.playbackMode) {
            case SDAnimatedImagePlaybackModeNormal:
                shouldEnd = (nextFrameIndex == 0);
                break;
            case SDAnimatedImagePlaybackModeReverse:
                shouldEnd = (nextFrameIndex == (self.totalFrameCount - 1));
                break;
            case SDAnimatedImagePlaybackModeBounce:
                if (self.loopEnding && !self.shouldReverse) {
                    shouldEnd = (nextFrameIndex == 1);
                }
                break;
            case SDAnimatedImagePlaybackModeReversedBounce:
                if (self.loopEnding && self.shouldReverse) {
                    shouldEnd = (nextFrameIndex == (self.totalFrameCount - 2));
                }
                break;
            default:
                break;
