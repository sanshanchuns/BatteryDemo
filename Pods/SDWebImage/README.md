### 快手tag更新规范：
* SDWebImage 的新 feature 和 tag 都基于 kwai-dev 分支打出
* ~~例如5.0.2.1e，5.0.2 是咱们的 SDWebImage 基于的官方版本，1e 是咱们自己的小版本号。1e -> 1f -> 1g ...，如果有重大更新，升级前面的数据，例如 2a~~
* tag不要使用字母结尾，包含字母的 version 会被 cocoapods 计算为预发布版本。快手修改的开源库均为原本三位基础上多加一位，x.x.x.x 形式。
