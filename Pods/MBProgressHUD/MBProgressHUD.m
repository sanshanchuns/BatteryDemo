...
- (void)animateIn:(BOOL)animatingIn withType:(MBProgressHUDAnimation)type completion:(void(^)(BOOL finished))completion {
    ...
    // 这里做了自定义的修改
    [UIView animateWithDuration:0.43 delay:0. usingSpringWithDamping:0.6f initialSpringVelocity:0.f options:(animatingIn ? UIViewAnimationOptionCurveEaseOut : UIViewAnimationOptionCurveEaseIn) animations:animations completion:completion];
}
...
- (void)updateConstraints {
    ...
    CGFloat xMargin = self.xMargin;
    CGFloat yMargin = self.yMargin;
    NSMutableArray *bezelConstraints = [NSMutableArray array];
    NSDictionary *xMetrics = @{@"xMargin": @(xMargin)};
    NSDictionary *yMetrics = @{@"yMargin": @(yMargin)};
    ...
    [sideConstraints addObjectsFromArray:[NSLayoutConstraint constraintsWithVisualFormat:@"|-(>=xMargin)-[bezel]-(>=xMargin)-|" options:0 metrics:xMetrics views:NSDictionaryOfVariableBindings(bezel)]];
    [sideConstraints addObjectsFromArray:[NSLayoutConstraint constraintsWithVisualFormat:@"V:|-(>=yMargin)-[bezel]-(>=yMargin)-|" options:0 metrics:yMetrics views:NSDictionaryOfVariableBindings(bezel)]];
    ...
    [topSpacer addConstraint:[NSLayoutConstraint constraintWithItem:topSpacer attribute:NSLayoutAttributeHeight relatedBy:NSLayoutRelationGreaterThanOrEqual toItem:nil attribute:NSLayoutAttributeNotAnAttribute multiplier:1.f constant:yMargin]];
    [bottomSpacer addConstraint:[NSLayoutConstraint constraintWithItem:bottomSpacer attribute:NSLayoutAttributeHeight relatedBy:NSLayoutRelationGreaterThanOrEqual toItem:nil attribute:NSLayoutAttributeNotAnAttribute multiplier:1.f constant:yMargin]];
    ...
    [subviews enumerateObjectsUsingBlock:^(UIView *view, NSUInteger idx, BOOL *stop) {
        ...
        [bezelConstraints addObjectsFromArray:[NSLayoutConstraint constraintsWithVisualFormat:@"|-(>=xMargin)-[view]-(>=xMargin)-|" options:0 metrics:xMetrics views:NSDictionaryOfVariableBindings(view)]];
        // Element spacing
        if (idx == 0) {
...
- (void)updatePaddingConstraints {
    ...
    [self.paddingConstraints enumerateObjectsUsingBlock:^(NSLayoutConstraint *padding, NSUInteger idx, BOOL *stop) {
        ...
        // 如果外部赋值则使用外部的，否则使用默认的
        CGFloat paddingValue = self.yPadding ? : MBDefaultPadding;
...
- (void)setYMargin:(CGFloat)yMargin {
    if (yMargin != _yMargin) {
        _yMargin = yMargin;
        [self setNeedsUpdateConstraints];
    }
...
- (void)setCornerRadius:(CGFloat)cornerRadius {
    if (cornerRadius != _cornerRadius) {
        ...
        self.bezelView.layer.cornerRadius = _cornerRadius;
    }
}

- (void)setYPadding:(CGFloat)yPadding {
    if (yPadding != _yPadding) {
        _yPadding = yPadding;
        [self updatePaddingConstraints];
    }
}
...
- (UIView *)hitTest:(CGPoint)point withEvent:(UIEvent *)event {
    ...
    if (view == self || view == self.backgroundView) {
        if (self.shouldIgnoreEventOnSelf) {
            view = nil;
        }
    }
    return view;
